...
#define OSTF_KILLECHO 	1
#define OSTF_STARTECHO 	2
...
typedef struct 
{
	...
	uint16_t 					GPIO_Pin;
	enum ledState_t 	GPIO_State;		//Led durumu, suandaki durumu Kapali/Acik
...
extern userLed_t userLed;

extern uint32_t LedHandler(userLed_t *_led);
extern uint32_t LedHandlerStatic(userLed_t *_led);
